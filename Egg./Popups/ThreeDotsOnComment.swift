//
//  ThreeDotsOnComment.swift
//  Egg.
//
//  Created by Jordan Wood on 9/26/22.
//
...
class ThreeDotsOnComment: UIViewController {
    ...
    func setupUI() {
        ...
        if let linkBigButton = self.linkBigButton {
            ...
            if let linkIcon = self.linkIcon {
                linkIcon.image = UIImage(systemName: "doc.on.clipboard")?.applyingSymbolConfiguration(.init(pointSize: 18, weight: .regular, scale: .medium))?.image(withTintColor: Constants.textColor.hexToUiColor())
    ...
    @objc internal func ReportButtonPressed(_ button: UIButton) {
        ...
        if authorOFComment == userID {
            ...
            var alertController: AlertViewController = {
                let alertController = AlertViewController(title: "Confirmation", body: "Are you sure you wanna delete this comment? This action can't be undone.", titleFont: UIFont(name: Constants.globalFontBold, size: 13), bodyFont: UIFont(name: Constants.globalFontBold, size: 13), buttonFont: UIFont(name: Constants.globalFontBold, size: 13))
                ...
                let blockAction = AlertAction(title: "Delete", style: .custom(textColor: Constants.universalRed.hexToUiColor())) {
                    ...
                    if self.replyID == "" {
                        print("* deleting comment: \(self.commentID)")
                        self.db.collection("posts").document(self.authorOfPost).collection("posts").document(self.postID).collection("comments").document(self.commentID).delete()
                    } else {
                        print("* deleting reply: \(self.replyID)")
                        self.db.collection("posts").document(self.authorOfPost).collection("posts").document(self.postID).collection("comments").document(self.commentID).collection("comment_replies").document(self.replyID).delete()
                    }
...
//        saveShareLinkToClipboard()
        print("* copying text: \(mainText)")
        UIPasteboard.general.string = mainText
        self.dismiss(animated: true)
        SPAlert.present(title: "Comment saved to clipboard!", preset: .done, haptic: .success)
        
    }
    @objc internal func ShareButtonPressed(_ button: UIButton) {
        guard let link = URL(string: "https://www.brodies.app") else { return }
        var dynamicLinksDomainURIPrefix = "https://brodies.page.link/post?uid=\(self.authorOfPost)&posti=\(self.postID)&commenti=\(self.commentID)"
        if replyID != "" {
            dynamicLinksDomainURIPrefix = "\(dynamicLinksDomainURIPrefix)&replyi=\(self.replyID)"
        }
...
//        linkBuilder?.iOSParameters?.minimumAppVersion = "1.2.3"
        ...
        linkBuilder?.socialMetaTagParameters?.title = "View comment on Brodie's"
...
extension ThreeDotsOnComment: PanModalPresentable {
