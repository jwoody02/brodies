...
//postId: postId,
//commentId: commentID,
//reply: change.data().replyText,
//replyId: replyID,
//replyUserID: change.data().authorID,
//replyUserName: change.data().authorUserName,
//replyUserImage: change.data().authorProfilePic,
//originalComment: commentSnap.data().commentText
class ReplyMadeNotification {
    var reply: String = ""
...
class ReplyMadeCell: UITableViewCell {
    ...
    var result = ReplyMadeNotification()
    func styleCell() {
        ...
        let attrib = "replied: \"\(result.reply)\"  \(result.timeSince)"
        ...
        let arrWit = 30
        if result.replyUserImage == "" {
            profilePicButton.image = UIImage(named: "no-profile-img.jpeg")
        } else {
            downloadImage(with: result.replyUserImage)
        }
    ...
    func styleCellFor(notification: ReplyMadeNotification, index: Int) {
        self.result = notification
        self.index = index
        self.usernameLabel.text = notification.replyUserName
